type EnvVariableName = 'NODE_PORT' | 'DB_PORT' | 'DB_USER' | 'DB_PASSWORD' | 'DB_DATABASE_NAME' | 'DB_HOSTNAME'
...
function _getEnv(name: EnvVariableName, type: 'number' | 'string') {
    switch (type) {
        case 'number': {
            ...
            if (!envVar || envVar.length <= 0 || isNaN(Number(envVar))) throw new Error(`Missing required environment variable ${name} ${envVar}`)
            return Number(envVar)
        }

        default: {
            const envVar = process.env[name]
            if (!envVar || envVar.length <= 0) throw new Error(`Missing required environment variable ${name} ${envVar}`)
            return envVar
        }
    }
}